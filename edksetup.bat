...
REM  edksetup.bat --NT32
REM  build -a IA32 -p Nt32Pkg\Nt32Pkg.dsc


set WORKSPACE=%CD%
REM set BASE_TOOLS_PATH=%WORKSPACE%\BaseTools\
REM set PYTHONPATH=%BASE_TOOLS_PATH%\Source\Python
REM set EDK_TOOLS_BIN=%WORKSPACE%\BaseTools\bin\edk2-BaseTools-win32\
set EDK_TOOLS_BIN=%WORKSPACE%\BaseTools\bin\win32\
REM set IASL_PREFIXC=%WORKSPACE%\BaseTools\bin\iasl-win-20180105\
REM set NASM_PREFIX=%WORKSPACE%\BaseTools\bin\nasm\
REM set WINSDK8x86_PREFIX=c:\Program Files (x86)\Windows Kits\8.1\bin\
REM set WINSDK8_PREFIX=C:\Program Files (x86)\Windows Kits\8.1\bin\
...
REM build -a  X64  -p OvmfPkg\OvmfPkgX64.dsc  -D DEBUG_ON_SERIAL_PORT
REM cd c:\qemu\
REM setup-qemu-x64.bat | tee OvmfBuild.log

rem build -a IA32 -p Nt32Pkg\Nt32Pkg.dsc   run
